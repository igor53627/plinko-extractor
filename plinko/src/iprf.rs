...
/// Metadata for a single SR level with precomputed round keys
struct SrLevelMeta {
    n: u64,
    round_keys: Vec<u64>,
}
...
/// with 128-bit security by default.
///
/// Optimization: Round keys are precomputed at construction time since they depend
/// only on (level, round, n) which are deterministic. This halves the AES calls
/// during forward/inverse operations (only prf_bit needs per-call AES).
...
impl SwapOrNotSrTee {
    ...
    pub fn with_security(key: PrfKey128, domain: u64, security_bits: u32) -> Self {
        ...
        let levels = Self::precompute_round_keys(&cipher, domain, max_levels, security_bits);
    ...
    fn precompute_round_keys(
        ...
        for level in 0..max_levels {
            ...
            let mut round_keys = Vec::with_capacity(t);
            ...
            for round in 0..t {
                ...
                let key = u64::from_be_bytes(block[0..8].try_into().unwrap()) % n;
                round_keys.push(key);
            }

            levels.push(SrLevelMeta { n, round_keys });
            n /= 2;
        }
    ...
    fn round_ct_with_key(&self, level: usize, round_num: usize, n: u64, x: u64, k_i: u64) -> u64 {
    ...
    fn apply_rounds_forward(&self, level: usize, x: u64) -> u64 {
        if level >= self.levels.len() {
            return x;
        }
        let meta = &self.levels[level];
        let mut val = x;
        for (r, &k_i) in meta.round_keys.iter().enumerate() {
            val = self.round_ct_with_key(level, r, meta.n, val, k_i);
        }
        val
    }

    fn apply_rounds_inverse(&self, level: usize, y: u64) -> u64 {
        if level >= self.levels.len() {
            return y;
        }
        let meta = &self.levels[level];
        let mut val = y;
        for (r, &k_i) in meta.round_keys.iter().enumerate().rev() {
            val = self.round_ct_with_key(level, r, meta.n, val, k_i);
        }
        val
    }
    ...
    pub fn forward(&self, x: u64) -> u64 {
        ...
        for level in 0..self.levels.len() {
            ...
            val = ct_select_u64(should_skip, val, self.apply_rounds_forward(level, val));
    ...
    pub fn inverse(&self, y: u64) -> u64 {
        use crate::constant_time::{ct_eq_u64, ct_ge_u64, ct_select_u64};

        debug_assert!(y < self.domain, "y must be < domain");
        let y = y % self.domain;

        let mut active_mask = [0u64; 64];
        let mut n = self.domain;
        let mut stopped: u64 = 0;

        for i in 0..self.levels.len() {
            let should_record = if n > 1 { 1u64 } else { 0u64 } & (1 - stopped);
            active_mask[i] = should_record;
        ...
        for level in (0..self.levels.len()).rev() {
            let skip = ct_eq_u64(active_mask[level], 0);
            val = ct_select_u64(skip, val, self.apply_rounds_inverse(level, val));
        }
        val
    }

    /// Batched inverse for multiple candidates - processes all in lockstep.
    ///
    /// This is significantly faster than calling `inverse()` N times because
    /// it batches AES operations using `encrypt_blocks`, leveraging AES-NI
    /// parallelism (8 blocks at a time).
    ///
    /// # Constant-time guarantee
    /// All candidates follow the same control flow. Per-candidate "active" masks
    /// handle the SR level logic without branching on secret data.
    pub fn inverse_batch(&self, ys: &mut [u64]) {
        use crate::constant_time::{ct_ge_u64, ct_lt_u64, ct_select_u64};

        let n_cand = ys.len();
        if n_cand == 0 || self.levels.is_empty() {
            return;
        }

        let num_levels = self.levels.len();
        let mut active_masks: Vec<Vec<u64>> = vec![vec![0u64; n_cand]; num_levels];

        for i in 0..n_cand {
            let y = ys[i] % self.domain;
            ys[i] = y;
            ...
            for level in 0..num_levels {
                let should_record = if n > 1 { 1u64 } else { 0u64 } & (1 - stopped);
                active_masks[level][i] = should_record;
                ...
                n = ct_select_u64(should_record & (1 - stopped), half, n);
            }
        }

        let mut blocks: Vec<GenericArray<u8, aes::cipher::typenum::U16>> =
            vec![GenericArray::default(); n_cand];
        let mut partners = vec![0u64; n_cand];
        ...
        for level in (0..num_levels).rev() {
            ...
            for (r, &k_i) in meta.round_keys.iter().enumerate().rev() {
                for i in 0..n_cand {
                    let x = ys[i] % n;
                    let partner = (k_i + n - x) % n;
                    let canonical = ct_select_u64(ct_lt_u64(x, partner), partner, x);
                    partners[i] = partner;
                ...
                for i in 0..n_cand {
                    let active = active_masks[level][i];
                    let bit = (blocks[i][0] & 1) as u64;
                    let should_swap = bit & active;
                    ys[i] = ct_select_u64(should_swap, partners[i], ys[i]);
                }
            }
        }
    }
...
///
/// The binomial sampler uses a parameterized iteration bound equal to `domain`
/// (which equals `total_hints` in Plinko). This is a PUBLIC protocol parameter,
/// so timing depends only on public values, preserving constant-time security.
pub struct IprfTee {
    #[allow(dead_code)]
    key: PrfKey128,
    cipher: Aes128,
    ...
    range: u64,
    tree_depth: usize,
    binomial_sampler: crate::binomial_tee::BinomialSamplerTee,
}

impl IprfTee {
    /// Creates a new TEE-safe iPRF instance with default 128-bit security.
    ///
    /// The binomial sampler iteration bound is set to `n` (the domain size),
    /// which equals `total_hints` in Plinko usage. This is a PUBLIC protocol
    /// parameter, so constant-time security is preserved.
    ///
    /// # Security Note
    /// The iteration count depends only on `n` (public), not on secret values
    /// like PRF outputs or ball counts derived from the key.
    pub fn new(key: PrfKey128, n: u64, m: u64) -> Self {
        Self::with_security(key, n, m, DEFAULT_SECURITY_BITS)
    }

    /// Creates a new TEE-safe iPRF instance with configurable PRP security.
    ///
    /// # Parameters
    /// - `security_bits`: PRP security level (e.g., 32, 64, 128). Higher = more rounds.
    ///   For testing/benchmarking, 32 is fast. For production, use 128.
    pub fn with_security(key: PrfKey128, n: u64, m: u64, security_bits: u32) -> Self {
        ...
        // Parameterized binomial sampler: iteration bound = domain (public)
        let binomial_sampler = crate::binomial_tee::BinomialSamplerTee::new(n);
    ...
    ///
    /// Uses batched PRP inverse for ~5-8x speedup on AES operations.
    ///
    /// # Precondition
    ...
    fn trace_ball_inverse_ct(&self, y: u64) -> (u64, u64) {
        ...
        for _level in 0..self.tree_depth {
            ...
            // Use parameterized TEE binomial sampler (iteration bound = domain, public)
            let left_count =
...
impl Iprf {
    ...
    ///
    /// When compiled with the `parallel` feature, PRP inverse calls are parallelized using rayon.
    pub fn inverse(&self, y: u64) -> Vec<u64> {
        if y >= self.range {
            return vec![];
        }
...
mod tests {
    ...
    #[test]
    fn test_sr_tee_inverse_batch_matches_scalar() {
        let key = [42u8; 16];
        let domain = 128u64;
        let prp = SwapOrNotSrTee::with_security(key, domain, 32);

        let mut ys_batch: Vec<u64> = (0..domain).collect();
        let ys_scalar: Vec<u64> = (0..domain).map(|y| prp.inverse(y)).collect();

        prp.inverse_batch(&mut ys_batch);

        for i in 0..domain as usize {
            assert_eq!(
                ys_batch[i], ys_scalar[i],
                "inverse_batch mismatch at y={}: batch={}, scalar={}",
                i, ys_batch[i], ys_scalar[i]
            );
        }
    }

    #[test]
    fn test_sr_tee_inverse_batch_correctness() {
        let key = [99u8; 16];
        let domain = 64u64;
        let prp = SwapOrNotSrTee::with_security(key, domain, 32);
        ...
        for x in 0..domain {
            let y = prp.forward(x);
            let mut ys = [y];
            prp.inverse_batch(&mut ys);
            assert_eq!(
                ys[0], x,
                "inverse_batch(forward({})) = {}, expected {}",
                x, ys[0], x
            );
        }
    }
